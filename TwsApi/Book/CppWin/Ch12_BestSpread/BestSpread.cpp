...
BestSpread::BestSpread(const char *host, int port, int clientId) :
signal(1000),
conId(-1),
...
BestSpread::~BestSpread() { delete reader; }

// Obtain contract ID
void BestSpread::contractDetails(int reqId, const ContractDetails& details) {
conId = details.contract.conId;
}

// Obtain current price
void BestSpread::tickByTickMidPoint(int reqId, time_t time, double midPoint) {
currentPrice = midPoint;
}

// Read strike prices and expiration dates
void BestSpread::securityDefinitionOptionalParameter(int reqId, const std::string& exch,
...
void BestSpread::securityDefinitionOptionalParameterEnd(int reqId) {
...
void BestSpread::tickPrice(TickerId reqId, TickType field,
...
void BestSpread::tickSize(TickerId reqId, TickType field, int size) {
...
void BestSpread::error(int id, int code, const std::string& msg) {
