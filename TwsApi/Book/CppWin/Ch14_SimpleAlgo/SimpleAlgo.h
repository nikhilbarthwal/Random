...
enum class Sentiment { BULLISH, BEARISH, MIXED };

class SimpleAlgo : public EWrapper, public EClientSocket {

public:
SimpleAlgo(const char *host, int port, int clientId = 0);
~SimpleAlgo();
...
bool spyBullish;
Sentiment sentiment;
std::vector<Contract> scanResults;
std::vector<std::tuple<int, double, double>> shortList;
long orderId, conId;
int numStocks, index;
double funds, *rsLevels, **prices;
double xi[20], xiSqr[20], xiSum, xiSqrSum;
