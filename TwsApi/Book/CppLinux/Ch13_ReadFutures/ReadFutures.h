#ifndef READ_FUTURES_H
#define READ_FUTURES_H
...
class ReadFutures : public EWrapper, public EClientSocket {

public:
ReadFutures(const char *host, int port, int clientId = 0);
~ReadFutures();

// Containers
EReader *reader;
EReaderOSSignal signal;
std::string localSymbol, multiplier;
std::map<std::string, std::string> symbols;
std::vector<double> closeVec, lowVec, highVec;
std::vector<long long> volVec;
