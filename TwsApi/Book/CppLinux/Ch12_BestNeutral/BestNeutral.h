#ifndef BEST_NEUTRAL_H
#define BEST_NEUTRAL_H
...
class BestNeutral : public EWrapper, public EClientSocket {

public:
BestNeutral(const char *host, int port, int clientId = 0);
~BestNeutral();
...
long conId, atmIndex;
double currentPrice;
std::string exchange, expiration;
std::set<std::string> expirations;
std::set<double> strikes;
std::map<double, std::map<char, Option>> chain;
std::map<double, double> beliefs;
